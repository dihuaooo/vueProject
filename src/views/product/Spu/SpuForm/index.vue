...
export default {
  data() {
    return {
      ...
      dialogVisible: false,
      // 存储spu信息属性
      // spu属性初始化的时候是一个空对象 在修改spu的时候 会向服务器发请求 返回spu的信息（对象） 在修改的时候
      // 可以利用服务器返回的这个对象收集最新的数据提交给服务器
      // 添加spu 如果是添加spu的时候并没有向服务器发请求 数据搜集到哪里【spu】 收集数据的时候有哪些字段 看文档
      spu: {
        // 三级分类的id
        category3Id: 0,
        // 描述
  ...
  methods: {
    ...
    handleRemove(file, fileList) {
      // file：代表的是删除的那个图片   fileList：照片墙删除某一张图片以后，剩余的其他图片
      console.log(file, fileList);
      // 收集照片墙图片的数据
      // 对于已有的图片【照片墙中显示的图片 有name、url字段的】 因为照片墙显示数据务必要有这两个属性
      // 对于服务器而言 不需要name、url字段 将来对于有的图片的数据在提交给服务器的时候需要处理
    ...
    addSaleAttrValue(row){
      ...
      // console.log(row)
      // 挂载在销售属性身上的响应式数据inputVisible 控制button和input切换
    ...
    async addOrUpdateSpu(){
      // 整理参数：需要整理照片墙的数据
      // 携带参数：对于图片 需要携带imageName 和 imgUrl字段
    ...
    cancel(){
      ...
      // 清理数据
      // Object.assign：合并对象
      // 组件实例的this._data可以操作data当中响应式数组
