<template>
  ...
  -->
    ...
    total：分页器一共需要展示数据条数
    page-size：代表每一页展示的条数
    page-sizes：可以设置每一页展示的条数
    layout：可以实现分页器的布局
    page-count:按钮的数量 如果 9 连续的页码是7 
  -->
    <el-pagination
...
export default {
  ...
  methods: {
    ...
    async getPageList(pager = 1) {
      ...
      if (result.code == 200) {
        // 分别是展示数据的总条数和展示数据的list
        this.total = result.data.total;
        this.list = result.data.records;
      }
    },
    // 页数变化时的提供的函数 有个pager属性 会将你点击的页码数回传
    ...
    // 当分页器某一页需要展示数据条数发生变化的时候会触发 有个limit 会将改变的数据条数回传
    handleSizeChange(limit) {
      this.limit = limit;
      this, this.getPageList();
    },
    // 点击添加品牌的按钮
    showDialog() {
      // 显示对话框
    ...
    updataTradeMark(row) {
      // row:当前用户选中这个品牌的信息
      // console.log(row)
      // 将已有的品牌信息赋值tmForm进行展示
      // 将服务器返回的品牌的信息直接赋值给了tmForm进行展示
